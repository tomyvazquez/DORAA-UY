...
import random
import time
# Importar el manejador de señales
def obtener_frase_graciosa():
    frases = [
        "\n¿Qué pasó Tomy, no anda la GNN?",
        "\nSanti, parece que la red neuronal está más confundida que tú.",
        "\nTomi, tu GNN tiene más bugs que un jardín en primavera.",
        "\nSanti, creo que la GNN se fue de vacaciones.",
        "\nTomy, volve a estudiar matemáticas."
    ]
...
def objective(trial):
    ...
    # if cfg.training.loss == 'normal':
    criterion = my_loss
    # elif cfg.training.loss == 'nn':
    #     criterion = my_loss_net_pf
        # pf_net = load_pf_net(device)
    # Entrenar el modelo
